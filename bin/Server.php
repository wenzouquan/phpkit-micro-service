...
//获得类构造函数参数
    ...
    function getServiceName($serv, $fd, $from_id, $data)
    {
        ...
        $fname = null;
        $mtype = 0;
        $r = $protocol->readMessageBegin($fname, $mtype, $rseqid);
    ...
    {
        if($this->tMultiplexedProcessor){
            return $this->tMultiplexedProcessor;
        }else{
            $arr = explode("\\", $serviceName);
            $name = $arr[count($arr) - 1];
            $processor_class = $serviceName . "\\" . $name . 'Processor';
            // $handler = new HiService();
            $processor = new $processor_class($this->handler);
